...
export type ProBadgeVariant = 'deliverer' | 'seller';
export type ProBadgeTone = 'solid' | 'soft';
export type ProBadgeSize = 'xs' | 'sm' | 'md';

export interface ProBadgeProps {
  /** `deliverer` affiche « VÉRIFIÉ », `seller` affiche « PRO ». */
  variant?: ProBadgeVariant;
  /** `solid` = pastille bleue pleine (défaut) ; `soft` = fond bleu pâle et bordure fine. */
  tone?: ProBadgeTone;
  size?: ProBadgeSize;
  /** Masque le libellé et ne garde que le sceau officiel vérifié. */
  iconOnly?: boolean;
...
export const VerifiedBadgeSvg: React.FC<{
  ...
  >
    <defs>
      <linearGradient id="proWebBlueGrad" x1="0%" y1="0%" x2="100%" y2="100%">
    ...
    <path
      d="M3.85 8.62a4 4 0 0 1 4.78-4.77 4 4 0 0 1 6.74 0 4 4 0 0 1 4.78 4.78 4 4 0 0 1 0 6.74 4 4 0 0 1-4.77 4.78 4 4 0 0 1-6.75 0 4 4 0 0 1-4.78-4.77 4 4 0 0 1 0-6.76Z"
      fill={tone === 'soft' ? '#1D4ED8' : 'url(#proWebBlueGrad)'}
    />
    <path
...
/**
 * Badge Vérifié / Pro — Bleu institutionnel (`#1D4ED8`).
 * Garantit une identité unifiée entre livreurs et vendeurs certifiés sur le web.
 */
export const ProBadge: React.FC<ProBadgeProps> = ({
  ...
  const s = SIZES[size];
  const text = label ?? (variant === 'seller' ? 'PRO' : 'VÉRIFIÉ');
  const title =
    variant === 'seller' ? 'Vendeur Pro vérifié' : 'Livreur vérifié par DaloaDelivery';
  ...
  if (iconOnly) {
    return (
      <span
        title={title}
        aria-label={title}
        className={cx('inline-flex items-center justify-center flex-shrink-0 select-none', className)}
      >
